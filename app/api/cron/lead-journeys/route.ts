...
import { processDueLeadJourneySteps } from "../../../lib/leadJourney";
...
export async function GET(req: Request) {
  ...
  try {
    const result = await processDueLeadJourneySteps({ limit });
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    console.error("Lead journey cron failed:", error);
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Lead journey cron failed.",
      },
      { status: 500 },
    );
  }
